...
- (void)updateView {
    ...
    for (NSString *str in _baseData) {
        ...
        row.onSelection = ^(UITableViewCell *cell) {
            [[[UIAlertView alloc] initWithTitle:@"SELECTED" message:str delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
        };
        row.onDeselection = ^(UITableView *cell) {
            [[[UIAlertView alloc] initWithTitle:@"DESELECTED" message:str delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
        };
        [rows addObject:row];
    }
